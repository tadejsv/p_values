...
# Some useful functions when working with games with many players

def array_to_func(array_in):
    def ret_func(ind):
        return(array_in[tuple(ind)])
    return(ret_func)

def func_to_array(func_in, shape):
    array = np.zeros(shape)
    for ind, val in np.ndenumerate(array):
        array[ind] = func_in(ind)
    return(array)

# This will get the array of full player payoffs based on input from the app
def full_payoffs(app_payoffs):
    payoffs = []
    num_players = len(app_payoffs)

    # This assumes that the number of moves is the same for all players
    num_moves = len(app_payoffs[0])

    for ind, player_app_payoff in enumerate(app_payoffs):
        next_ind = (ind + 1) if (ind + 1) < num_players else 0
        indices = [ind, next_ind]

        def payoff_func(ind):
            new_ind = np.array(ind)[indices]
            return(player_app_payoff[tuple(new_ind)])

        payoffs.append(func_to_array(payoff_func, [num_moves]*num_players))

    return payoffs

# This will get the array of full probabilities based on input from the app
def full_probabilities(app_probs):
    num_players = len(app_probs)

    # This assumes that the number of moves is the same for all players
    num_moves = len(app_probs[0])

    def prob_r(ind):
        temp_prob = 1
        for player, move in enumerate(ind):
            temp_prob *= app_probs[player][move]

        return(temp_prob)

    return func_to_array(prob_r, [num_moves]*num_players)
